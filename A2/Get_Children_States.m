function [ children ] = Get_Children_States( current_node )
...
%     if dir == 0
%         if cs(1)+1 < 4
%            states = [states; [[cs(1)+1, cs(2), 0], FORWARD]];
%         end
%         states = [states; [[cs(1), cs(2), 1], LEFT]; [[cs(1), cs(2), 3], RIGHT]];
%     elseif dir == 1
%         if cs(2)+1 < 4
%             states = [states; [[cs(1), cs(2)+1, 1], FORWARD]];
%         end
%         states = [states; [[cs(1), cs(2), 2], LEFT]; [[cs(1), cs(2), 0], RIGHT]];
%     elseif dir == 2
%         if cs(1)-1 > 1
%            states = [states; [[cs(1)-1, cs(2), 2], FORWARD]]; 
%         end
%         states = [states; [[cs(1), cs(2), 3], LEFT]; [[cs(1), cs(2), 1], RIGHT]];
%     elseif dir == 3
%         if cs(2)-1 > 1
%            states = [states; [[cs(1), cs(2)-1, 3], FORWARD]]; 
%         end
%         states = [states; [[cs(1), cs(2), 0], LEFT]; [[cs(1), cs(2), 2], RIGHT]];
%     end
